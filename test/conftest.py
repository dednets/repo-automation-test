...
class GitHubTestManager:
    ...
    def initialize_test_repository(self, repo_config: RepositoryConfig) -> bool:
        ...
        try:
            ...
            import shutil
            github_backup = temp_init_path / ".github_backup"
            
            for item in current_repo.iterdir():
                if item.name in ['.git', '__pycache__', '.pytest_cache', 'venv', 'cache']:
                    continue
                elif item.name == '.github':
                    # Save .github for later
                    shutil.copytree(item, github_backup)
                    print(f"Saved .github directory for later deployment")
                else:
                    if item.is_dir():
                        shutil.copytree(item, temp_init_path / item.name)
                    else:
                        shutil.copy2(item, temp_init_path / item.name)
            ...
            subprocess.run(["git", "add", "."], cwd=temp_init_path, check=True)
            subprocess.run(["git", "commit", "-m", "Initial commit - source code without workflows"], cwd=temp_init_path, check=True)
            ...
            # Now restore .github directory and push it if the initial push succeeded
            github_backup = temp_init_path / ".github_backup"
            if github_backup.exists():
                github_dir = temp_init_path / ".github"
                github_backup.rename(github_dir)
                
                # Stage and commit .github directory
                subprocess.run(["git", "add", ".github/"], cwd=temp_init_path, check=True)
                subprocess.run(["git", "commit", "-m", "Add GitHub Actions workflows and configuration"], cwd=temp_init_path, check=True)
                
                print(f"Pushing GitHub Actions workflows to {repo_config.full_name}...")
                ...
                if workflow_result.returncode != 0:
                    print(f"⚠️ Warning: Could not push workflow files: {workflow_result.stderr}")
                    print("💡 This might be due to fine-grained token limitations with workflow files.")
                    print("   Tests will still work, but workflows won't be available in the test repository.")
                else:
                    print(f"✅ Successfully pushed GitHub Actions workflows")
